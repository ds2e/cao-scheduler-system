...
export default function SideCheckoutTable({
    isLoading,
    itemsList,

    requestTransferItemAmountToMainCheckout,
    requestTransferItemToMainCheckout,
    requestCheckoutSideTable
}) {
    ...
    return (
        <div className="flex flex-col gap-y-2 h-full justify-between">
            <h2
                className={`text-theme bg-gray-200 font-bold p-2 text-center rounded-full border-2 border-theme`}
            >
                getrennte Rechnung
            </h2>

            <div className="flex flex-col bg-black h-[80%] overflow-y-auto">
                {itemsList.map((co_item_side, co_item_side_ind) => {
                    return (
                        <div key={String(co_item_side.id) + co_item_side_ind + "order"}>
                            <div
                                className={`w-full flex justify-between bg-gray-900`}
                            >
                                <button
                                    onClick={() => requestTransferItemAmountToMainCheckout(co_item_side.id)}
                                    className="flex gap-x-2 items-center bg-gray-700 rounded-r-full px-4 py-2">
                                    {co_item_side.code && <span className="text-sm text-gray-200">{co_item_side.code}</span>}
                                    <span className="text-white">{co_item_side.name}</span>
                                </button>

                                <div className="flex gap-x-2">
                                    <div className="flex gap-x-2 items-center">
                                        <span className="text-sm text-white">{co_item_side.price}&euro;</span>
                                        <span
                                            className={`font-bold text-theme-secondary`}
                                        >
                                            x{co_item_side.amount}
                                        </span>
                                    </div>
                                    <button
                                        onClick={() => requestTransferItemToMainCheckout(co_item_side.id)}
            ...
            <button
                ...
                onClick={() => requestCheckoutSideTable()}
